...
class AstroSpellChecker {
    constructor() {
        ...
        this.ignoredWords = [
            'SYCIT','SICYT','UdeG', 'UDG', 'UNE', 'ASTRO', 'JSX', 'CSS', 'HTML', 'JS', 'TS',
            'API', 'URL','RVOE','VLEX','Card','ELibro','slide','padding', 'ESP','SEO', 'CMS', 'UI', 'UX', 'GITHUB', 'NPM',
    ...
    extractTextContent(content, filePath) {
        ...
        if (path.extname(filePath) === '.astro') {
            ...
            processedContent = processedContent.replace(/<script[\s\S]*?<\/script>/gi, ''); // Scripts
            processedContent = processedContent.replace(/<style[\s\S]*?<\/style>/gi, ''); // Styles
    ...
    async askCorrectionAction(correction, fileContext) {
        ...
        if (this.correctionMemory.has(originalWord)) {
            ...
            if (savedDecision.action === 'apply') {
                console.log(`✅ Aplicando sugerencia recordada: "${correction.original}" → "${savedDecision.replacement}"`);
                return {
                    action: 'apply',
                    replacement: savedDecision.replacement
                };
            } else if (savedDecision.action === 'custom') {
                console.log(`✏️  Aplicando corrección personalizada recordada: "${correction.original}" → "${savedDecision.replacement}"`);
                return {
                    action: 'custom',
                    replacement: savedDecision.replacement
                };
            } else {
                console.log(`⏭️  Ignorando (decisión previa): "${correction.original}"`);
                return {
                    action: 'ignore',
                    replacement: null
                };
        ...
        return new Promise((resolve) => {
            ...
            this.rl.question('\nElige una opción [1/2/3]: ', (answer) => {
                ...
                if (choice === '1') {
                    ...
                    this.rl.question(`\n✏️  Escribe la corrección para "${correction.original}": `, (customCorrection) => {
        ...
        try {
            ...
            if (corrections.length > 0) {
                ...
                const textErrors = corrections.filter(correction => {
                    ...
                    if (this.ignoredWords.some(word => 
                        originalExact.toUpperCase() === word.toUpperCase())) {
                        return false;
                    }
                    ...
                    return /^[a-záéíóúñü\s]+$/i.test(original) && 
                           original.length >= 3 && 
                           original.length <= 15;
                });
                
                if (textErrors.length === 0) {
                    console.log('   ℹ️  Solo errores de formato/código encontrados (ignorados)');
                    return { file: filePath, hasChanges: false };
                }
                ...
                for (const [index, correction] of textErrors.entries()) {
                    console.log(`\n${'═'.repeat(70)}`);
                    console.log(`📍 ERROR ${index + 1} de ${textErrors.length} en: ${path.basename(filePath)}`);
                    console.log(`🔍 Tipo: ${correction.rule} (${correction.ruleId})`);
                    console.log(`${'═'.repeat(70)}`);
                    
                    // Mostrar contexto mejorado
                    const contextStart = Math.max(0, correction.context.indexOf(correction.original) - 30);
                    const contextEnd = Math.min(correction.context.length, correction.context.indexOf(correction.original) + correction.original.length + 30);
                    const contextSnippet = correction.context.substring(contextStart, contextEnd);
                    ...
                    if (correction.ruleId === 'UPPERCASE_SENTENCE_START') {
                        console.log('💡 Explicación: Las oraciones deben empezar con mayúscula');
                    } else if (correction.ruleId === 'MORFOLOGIK_RULE_ES') {
                        console.log('💡 Explicación: Posible error de ortografía o falta de acento');
                    } else if (correction.ruleId.includes('QUESTION') || correction.ruleId.includes('EXCLAMATION')) {
                        console.log('💡 Explicación: Las preguntas deben llevar ¿? y las exclamaciones ¡!');
                    } else if (correction.ruleId.includes('WHITESPACE')) {
                        console.log('💡 Explicación: Espaciado incorrecto');
                    }
                    
                    const userAction = await this.askCorrectionAction(correction, contextSnippet);
                    ...
                    if (userAction.action === 'apply') {
                        ...
                        console.log(`✅ Aplicando sugerencia: "${correction.original}" → "${userAction.replacement}"`);
                        ...
                        console.log(`✏️  Aplicando corrección personalizada: "${correction.original}" → "${userAction.replacement}"`);
                ...
                if (acceptedCorrections.length > 0) {
                    ...
                    const finalContent = this.restoreProtectedParts(correctedText, protectedParts, originalContent);
    ...
    async run() {
        ...
        const jarPath = "C:\\languagetool\\LanguageTool-6.6\\languagetool-commandline.jar";
        ...
        console.log('💡 Para cada error podrás elegir: [1] Aplicar sugerencia [2] Escribir tu corrección [3] Ignorar');
        ...
        console.log('\n' + '='.repeat(80));
        console.log(`📋 RESUMEN FINAL: ${filesWithChanges.length} archivo(s) con correcciones aprobadas`);
        console.log('='.repeat(80));
        
        filesWithChanges.forEach(change => {
            console.log(`📄 ${change.file}: ${change.corrections.length} corrección(es) aprobadas`);
        });
